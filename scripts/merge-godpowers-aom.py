#!/usr/bin/env python3
"""
Atualiza godpowers.json com dados extraídos do jogo.

Campos preservados (não sobrescritos):
  id, god, era, panteao, descricao_avancada
...
from aom_godpower_merge import (
...
def locale_paths(locale: str) -> Path:
    ...
    if locale_key == "en":
        return ROOT / "src" / "data" / "locale" / "en" / "godpowers.json"
    return ROOT / "src" / "data" / "locale" / "pt" / "godpowers.json"
...
def main() -> int:
    ...
    parser.add_argument(
        "--power",
        help="Nome(s) em inglês ou proto separados por vírgula (ex.: Bolt ou Lightning Storm)",
    )
    ...
    parser.add_argument(
        ...
        choices=("pt", "en", "both"),
        default="both",
        help="Qual godpowers.json atualizar (padrão: both)",
    )
    ...
    power_filter = parse_power_filter(args.power)
    id_filter = parse_id_range(args.ids)
    if power_filter is None and id_filter is None:
        print("Informe --power ou --ids para limitar o merge.", file=sys.stderr)
        return 1
    ...
    extracted_rows = extract_godpowers(cache_dir)
    ...
    print(
        ...
        f"god powers no jogo: {len(extracted_index)}"
        f"{f' (+{added} sob demanda)' if added else ''} | "
        f"filtro: power={args.power or '-'} ids={args.ids or '-'}"
    )
