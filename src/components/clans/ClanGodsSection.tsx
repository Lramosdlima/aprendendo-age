...
import type { ClanGodAggregate } from "@/lib/clanGodsApi";
...
function ClanGodCard({
  ...
  return (
    <article
      className={cn(
        "flex flex-col items-center rounded-2xl border bg-zinc-950/55 text-center shadow-lg shadow-black/30 backdrop-blur-sm",
        theme.accentBorder,
      ...
      {loading ? (
        ...
        <>
          ...
          <ul className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
            {topGods.map((god) => (
              <li key={god.slug}>
                <ClanGodCard god={god} theme={theme} t={t} />
              </li>
            ))}
          </ul>
          ...
          {showAll ? (
            <ul className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {gods.map((god) => (
                <li key={god.slug}>
                  <ClanGodCard god={god} theme={theme} compact t={t} />
                </li>
              ))}
            </ul>
          ) : null}
